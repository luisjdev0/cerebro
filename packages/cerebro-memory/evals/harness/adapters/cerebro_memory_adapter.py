"""
Adaptador de cerebro-memory (el sistema propio) contra la API HTTP real.

A diferencia de NaiveKeywordAdapter (índice en RAM) o de un mem0/graphiti/letta
(SDK en proceso), este adaptador habla con `cerebro_memory.api` por HTTP -- exactamente
como lo haría el servidor MCP (`cerebro_memory.mcp_server`) o cualquier otro cliente
real. Requiere que la API esté corriendo (ver README.md, `python -m cerebro_memory.main`)
...
escenario que cerebro-memory ni siquiera puede producir (dos memorias "activas" en
...
class CerebroMemoryAdapter(MemoryAdapter):
    """Adaptador contra la API real de cerebro-memory (`src/cerebro_memory/api.py`) vía httpx."""
