"""CLI de cerebro-memory: `cerebro-memory <subcomando>` (entry point en pyproject.toml).

Como el servidor MCP (`cerebro_memory.mcp_server`), es un cliente delgado de la API HTTP
-- ninguna lógica de negocio nueva aquí salvo la orquestación propia del importador de
Markdown (parsing puro vive en `cerebro_memory.markdown_importer`). La única excepción es
...
Config: lee `.env` (vía `cerebro_memory.config.get_settings`, igual que la API) para
...
from cerebro_memory.db import PACKAGE_ROOT
from cerebro_memory.markdown_importer import ParsedMemory, iter_markdown_files, parse_markdown_file

# monorepo root (packages/cerebro-memory/../..): compose.yaml, y por tanto el
# contexto de `docker compose exec`, vive ahi - no en la raiz de este paquete.
...
def _api_client() -> httpx.Client:
    return httpx.Client(
        ...
        headers={
            ...
            "X-Agent-Name": "cerebro-memory-cli",
        },
        timeout=30.0,
    )
...
def _connection_error(exc: httpx.RequestError) -> str:
    return (
        f"No se pudo conectar con la API de cerebro-memory en {_api_base_url()}: {exc}. "
        "Verifica que este corriendo (`python -m cerebro_memory.main`)."
    )
...
def cmd_backup(args: argparse.Namespace) -> None:
    ...
    out_file = out_dir / f"cerebro-memory-{timestamp}.sql"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cerebro-memory", description="CLI de cerebro-memory")
    ...
    p_restore.add_argument("file", help="archivo .sql generado por 'cerebro-memory backup'")
